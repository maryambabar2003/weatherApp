...
sendMessageBtn.addEventListener('click', async () => {
  ...
  if (userMessage.toLowerCase().includes('weather')) {
    const city = userMessage.split(' ').pop(); // Extract city name from the message
    const weatherData = await fetchWeatherData(city);
    if (weatherData) {
      displayMessage('Bot', `The weather in ${city} is ${weatherData.main.temp} °C with ${weatherData.weather[0].description}.`);
    } else {
...
async function fetchGeminiResponse(query) {
  const geminiApiKey = "AIzaSyBiz-sV8jgCC3s6UaaTE7JIWum3Bh2ld84"; // Your Gemini API key
  const geminiUrl = `https://api.gemini.ai/v1/assistant/${geminiApiKey}/query`;
